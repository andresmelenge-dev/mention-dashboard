    ...
    <div style={{ background: rowBg, border: `1px solid ${COLORS.border}`, borderRadius: 10, padding: 16, marginBottom: 10 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12 }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <p style={{ color: '#cccccc', fontSize: 13, margin: 0, lineHeight: 1.5, wordBreak: 'break-word' }}>
            {item.message}
          </p>
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8, flexShrink: 0 }}>
          <a href={item.link} target="_blank" rel="noreferrer" style={{
            background: COLORS.accent, color: '#0f0f1a', padding: '7px 14px',
            borderRadius: 6, textDecoration: 'none', fontSize: 12, fontWeight: 'bold', whiteSpace: 'nowrap'
          }}>🔗 Ir al mensaje</a>
          <button onClick={() => onArchive(item)} style={{
            background: '#1a4a2a', color: COLORS.green, padding: '7px 14px',
            border: 'none', borderRadius: 6, cursor: 'pointer', fontSize: 12, fontWeight: 'bold', whiteSpace: 'nowrap'
          }}>✅ Gestionar</button>
        </div>
      </div>
    </div>
